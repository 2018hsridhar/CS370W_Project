// #TODO :: note that this version will perform alignment ACCORDING to the interpolating surface, instead of the objects themselves
// NEED to align based on cross product of { edge_vectors, object_normals } W.R.T. the boundary
//  --- how do I get this boundary though? it is a set of common vertices with object scans ( for sure ). 
...
bool key_down(igl::viewer::Viewer& viewer, unsigned char key, int mod)
{
	switch(key)
	{
		...
		case ' ':
		{
			...
			// #TODO :: shouldn't I be working with the average length of only the boundary edges, and NOT all edges? But, assuming my inputs possess reasonable aspect ratios, I think I should be fine here.
...
// #TODO :: note that we do run into the issue of splitting more edges than neccessary. We must keep edge lens ( lest we end up with extraneuous boundary vertices ). We need to keep the set of boundary vertices as it is
// #TODO :: do vertex or face normals make more sense here? Face normals might be difficult to compute. Easier to use vertex normals/edge normals, isn't it??? It has to be a face normal. With vertex normals, you'll always get things pointing in a straight direction. This would lead to an error in logic.

			bool remSucc = REMESH::remeshSurface(interp.V,interp.F,remeshed.V,remeshed.F, rEL);

			// check indices of [interp.V,remeshed.V] here for consistency
			std::string interp_output = "interpSurface.off";
			std::string remesh_output = "remeshedSurface.off";
			igl::writeOFF(interp_output, interp.V, interp.F);
			igl::writeOFF(remesh_output, remeshed.V, remeshed.F);
			return 0;
...
//			cout << "Flowing the interpolating surface" << endl;
			...
			// #NOTE :: in the MCF computation, we've the boolean vector ( igl::is_border_vertex )
...
// #NOTE :: this is a debug method . It's not really used in the codebase. 
void constructNormalField(Eigen::MatrixXd& V, Eigen::MatrixXd& N,
						Eigen::MatrixXd& V_res, Eigen::MatrixXi& N_edges)
{
